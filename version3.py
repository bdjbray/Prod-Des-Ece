...
class TweetAnalyzer():  #used for analyze the tweets
    def tweets_to_data_frame(self, tweets):      #store the tweets that we have got and to further operation
        ...
        df['date'] = np.array([tweet.created_at for tweet in tweets])  #we create different column for different information we want to display and display them
...
if __name__ == '__main__':
    ...
    tweets = api.user_timeline(screen_name="KingJames", count=20)  #choose the user we want to listen and the number of tweets we want to analyse
